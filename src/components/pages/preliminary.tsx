...
export function Preliminary() {
    ...
    const addAssessor = () => {
        if (!nameReference.current || !nameReference.current.value.trim())
            return;
        appState.value = {
            ...appState.value,
            assessors: [
                ...appState.value.assessors,
    ...
    return (
        <div className={"flex"}>
            ...
            <h3>Assessors</h3>
            {appState.value.assessors.length ? (
                <ul>
                    {appState.value.assessors.map((a) => (
                        <li>
                            {a} (
                            <a onClick={() => removeAssessor(a)} href="#">
                                remove
                            </a>
                            )
                        </li>
