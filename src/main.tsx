...
const form_data = Object.values(
    import.meta.glob("./assets/forms/*.jsonc", {
        query: "?raw",
        eager: true,
        import: "default"
    })
).map((c) => parse(c)) as Form[];
    ...
    return (
        <>
            {appState.value.selectedForm ? (
                ...
                <Modal title="Assessment Setup">
                    <p>
                        Please select the type of assessment you're conducting:
                    </p>
                    <select value={""} ref={dropdownReference}>
                        {form_data.map((form) => {
                            return (
                                <option value={form.id}>
                                    {form.source} - {form.name}
                                </option>
                            );
                        })}
                    </select>
            ...
            <footer onClick={() => setCopyrightOpen(true)}>
                v{version} &copy; 2026
            </footer>
            {copyrightOpen && (
                <Modal title="Copyright & Credits">
                    <p>Copyright &copy; 2026 Benjamin O'Brien</p>
                    <p>
                        Made possible by <a href="https://vite.dev">vite</a>,{" "}
                        <a href="https://preactjs.com">preact</a>, and many
                        others.
                    </p>
                    <p>
                        Source code available{" "}
                        <a href="https://github.com/iiPythonx/shield">
                            on GitHub
                        </a>
                        .
                    </p>
                    <button onClick={() => setCopyrightOpen(false)}>
                        Close
                    </button>
                </Modal>
            )}
